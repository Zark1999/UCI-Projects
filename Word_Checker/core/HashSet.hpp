...
private:
	...
	void rehashing(Node** source, int sc, Node** target, int tc);
	void deleteAll(Node** head, int capacity);
	Node** copyAll(Node** head, int capacity);
};
...
: hashFunction{hashFunction},num{0},capacity{DEFAULT_CAPACITY},arr{nullptr}
{
    ...
    : hashFunction{impl_::HashSet__undefinedHashFunction<ElementType>}
{
	...
	s.arr = new Node*[DEFAULT_CAPACITY];
	for (unsigned int i=0; i<DEFAULT_CAPACITY; i++)
	{
		s.arr[i] = nullptr;
	}
...
HashSet<ElementType>& HashSet<ElementType>::operator=(const HashSet& s)
{
	if (this != &s)
	{
		Node** new_arr = copyAll(s.arr,s.capacity);
		deleteAll(arr, capacity);
...
void HashSet<ElementType>::add(const ElementType& element)
{
	if (not this->contains(element))
	{
		...
		if (num > capacity*0.8)
		{
			...
			rehashing(arr,capacity,new_arr,capacity*2);
			deleteAll(arr, capacity);
			arr = new_arr;
			capacity = capacity*2;
		}
...
{
	Node* curr = arr[hashFunction(element) % capacity];
...
template <typename ElementType>
void HashSet<ElementType>::rehashing(Node** source, int sc, Node** target, int tc)
{
	for (unsigned int i=0; i<sc; i++)
	{
		...
		while (curr != nullptr)
		{
			int index = hashFunction(curr->element) % tc;
			Node* n = new Node{curr->element,nullptr};
			Node* curr2 = target[index];
			if (curr2 == nullptr)
			{
				target[index] = n;
			}
...
template <typename ElementType>
void HashSet<ElementType>::deleteAll(Node** head, int capacity)
{
...
template <typename ElementType>
typename HashSet<ElementType>:: Node** HashSet<ElementType>::copyAll(Node** head, int capacity)
{
